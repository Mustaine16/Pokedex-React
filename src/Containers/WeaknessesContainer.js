...
import Weaknesses from "../components/pokemonPage/Weaknesess/Weaknesses";
        ...
        await fetch(`${url}`)
          ...
          .then(res => {
            ...
            if (i === 0) {
              ...
              defDbArr.length > 0
                ? defDbArr.forEach(e => calculatedDamage.defense.double.push(e))
                : console.log("none double");
              defHalfArr.length > 0
                ? defHalfArr.forEach(e => calculatedDamage.defense.half.push(e))
                : console.log("none half");
              defNoneArr.length > 0
                ? defNoneArr.forEach(e => calculatedDamage.defense.none.push(e))
                : console.log("none none");
            }
          });
      }
        ...
        switch (value) {
          case "double":
            if (defDbArr.some(e => e === `${stat}`) && stat) {
              ...
              calculatedDamage.defense.quadruple.push(stat);
          ...
          case "half":
            if (defDbArr.some(e => e === `${stat}`)) {
              ...
              calculatedDamage.defense.quarter.push(stat);
            } else if (!(defNoneArr.some(e => e === stat) && stat)) {
              //Se pushea de manera normal
          ...
          case "none":
            if (defDbArr.some(e => e === `${stat}`)) {
              ...
              calculatedDamage.defense.none.push(stat);
            } else if (defHalfArr.some(e => e === `${stat}`)) {
              //Busca el indice del elemento y luego lo elimina en Half
              let i = defHalfArr.indexOf(stat);
              defHalfArr.splice(i, 1);
